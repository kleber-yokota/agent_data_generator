...
llm1 = ChatOllama(model="qwen3:8b", temperature=1.0)
...
Return the results in YAML format, like this:
...
column: status
  proportion: {"active": 0.8, "inactive": 0.2}
...
general:
  ...
  proportion: {"male": 0.5, "female": 0.5}
...
def detect_intention(state: ExtractionAgent) -> ExtractionAgent:
    ...
    state["qty_map"] = range(round(state["metadata"]["rows"]/10.,0))
...
- If unable to generate all 30 rows, output as many as possible, but ensure that:
...
result = graph.invoke({
    "user_input": "gere um metadados de cadastro de usuario, com 8 colunas e 1000 registro"
})
