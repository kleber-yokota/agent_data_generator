...
docs = [simple_tokenize(doc["text"]) for doc in raw_dataset]
...
checkpoint_path = "doc2vec_dbow_complete.pth"
checkpoint = {
...
model_loaded, word2idx_loaded = load_model(checkpoint_path, device)
...
def print_similar_docs(query_idx, docs, embeddings, top_k=5):
    if query_idx >= len(embeddings):
        print(f"Índice {query_idx} fora do intervalo. Total de docs: {len(embeddings)}")
        return
