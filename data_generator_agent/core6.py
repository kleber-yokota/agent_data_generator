...
- If unable to generate all 50 rows, output as many as possible, but ensure that:
...
def split_generate_jobs(state: dict) -> list[Send]:
    ...
    total_rows = 100 if rows is None else rows  # fallback
    chunk_size = 20
...
def generate_chunk_data(state: dict) -> dict:
    ...
    print(f"chunck: {state["chunk_index"]}")
    print(r)
    return {"csv_chuck":[r]}
...
def reduce_generated_data(states: list[dict]) -> dict:
    full_csv = "\n".join(states["csv_chuck"])
    df = pd.read_csv(StringIO(full_csv),header=False)
    cols = [col["name"] for col in states["metadata"]]
    df.columns = cols
    df.to_csv("core6.csv",index=False)
